...
from sciona.probes.scipy.linalg import probe_records
...
def test_probe_records_publish_expected_linalg_symbols() -> None:
    wrapper_symbols = {str(record["wrapper_symbol"]) for record in probe_records()}
    assert wrapper_symbols == {"solve", "inv", "det", "lu_factor", "lu_solve"}
