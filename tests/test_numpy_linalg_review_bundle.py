...
BUNDLE_PATH = ROOT / "data" / "review_bundles" / "numpy_linalg_pubrev_050.review_bundle.json"
...
EXPECTED_SOURCE_PATHS = {
    "src/sciona/atoms/numpy/linalg.py",
    "src/sciona/probes/numpy/linalg.py",
    "src/sciona/atoms/numpy/references.json",
    "tests/test_numpy_linalg_behavior.py",
    "tests/test_numpy_linalg_references_metadata.py",
    "tests/test_numpy_linalg_review_bundle.py",
}


def test_numpy_linalg_pubrev_050_review_bundle_is_publishable_with_limits() -> None:
    bundle = json.loads(BUNDLE_PATH.read_text(encoding="utf-8"))

    assert bundle["provider_repo"] == "sciona-atoms"
    assert bundle["family_batch"] == "numpy_linalg_pubrev_050"
    ...
    limited_atoms = {"sciona.atoms.numpy.linalg.inv", "sciona.atoms.numpy.linalg.solve"}
    for row in rows:
        ...
        assert row["review_record_path"] == "data/review_bundles/numpy_linalg_pubrev_050.review_bundle.json"
        assert set(row["source_paths"]) == EXPECTED_SOURCE_PATHS
        for rel_path in row["source_paths"]:
            assert (ROOT / rel_path).exists()
        if row["atom_key"] in limited_atoms:
            assert any("stacked-matrix" in limitation for limitation in row["limitations"])


def test_numpy_linalg_pubrev_050_bundle_merges_as_approved_rows() -> None:
