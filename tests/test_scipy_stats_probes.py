...
from sciona.probes.scipy.stats import probe_records
...
def test_probe_records_publish_expected_stats_symbols() -> None:
    wrapper_symbols = {str(record["wrapper_symbol"]) for record in probe_records()}
    assert wrapper_symbols == {"describe", "norm", "pearsonr", "spearmanr", "ttest_ind"}
